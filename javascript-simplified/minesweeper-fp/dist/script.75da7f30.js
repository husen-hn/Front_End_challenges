...
exports.TILE_STATUSES = TILE_STATUSES;
...
function markedTilesCount(board) {
  return board.reduce(function (count, row) {
    return count + row.filter(function (tile) {
      return tile.status === TILE_STATUSES.MARKED;
    }).length;
  }, 0);
}

function markTile(board, _ref) {
  var x = _ref.x,
      y = _ref.y;
  var tile = board[x][y];
...
function replaceTile(board, position, newTile) {
  return board.map(function (row, x) {
    return row.map(function (tile, y) {
      if (positionMatch(position, {
        x: x,
        y: y
      })) {
        return newTile;
...
function revealTile(board, _ref2) {
  var x = _ref2.x,
      y = _ref2.y;
  var tile = board[x][y];
  ...
  var mines = adjacentTiles.filter(function (t) {
    return t.mine;
  });
  var newBoard = replaceTile(board, {
    x: x,
    y: y
  }, _objectSpread(_objectSpread({}, tile), {}, {
  ...
  if (mines.length === 0) {
    return adjacentTiles.reduce(function (b, t) {
...
function nearbyTiles(board, _ref3) {
  ...
  return offsets.flatMap(function (xOffset) {
    return offsets.map(function (yOffset) {
      ...
      return (_board = board[x + xOffset]) === null || _board === void 0 ? void 0 : _board[y + yOffset];
    });
...
function render() {
  ...
  checkGameEnd();
  getTileElements().forEach(function (element) {
...
function getTileElements() {
  return board.flatMap(function (row) {
